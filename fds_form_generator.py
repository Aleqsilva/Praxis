...
class FMAData(TrackplanElementData):
    ...
    def __init__(self, xml_element):
        ...
        # Carregar trilhos e links associados
        rails_refs = xml_element.findall(".//Rails/Ref")
        for rail_ref in rails_refs:
            rail_id = rail_ref.get("refId")
            if rail_id:
                self.associated_rails.append({
                    "refId": rail_id
                })
        
        # Carregar sensores associados
        sensors_refs = xml_element.findall(".//Sensors/Ref")
        for sensor_ref in sensors_refs:
            sensor_id = sensor_ref.get("refId")
            fma_position = sensor_ref.get("fmaPosition", "left")
            self.associated_sensors.append({
                "refId": sensor_id,
                "fmaPosition": fma_position
            })
...
class IntelligentXMLGenerator:
    ...
    def generate_smart_xml(self, filename):
        ...
        try:
            ...
            fds_elem.set("ip", self._get_form_value("IpAddressNet1", "192.168.1.25"))
            fds_elem.set("netmask", self._get_form_value("MaskNet1", "255.255.255.0"))
            ...
            real_rail_ids_in_xml = {str(r.get("id")) for r in real_rails} | {str(s.get("id")) for s in switches} | {str(l.get("id")) for l in links}
            ...
            for sensor in sensors:
                ...
                if sensor_id and sensor_id.startswith("2") and len(sensor_id) > 1:  # Validação melhorada
                    ...
                    print(f"Sensor {sensor_id} não segue o padrão esperado (2XXXX)")
                ...
                if not rail_at_position and position not in created_auto_rails:
                    ...
                    auto_rail_xml.set("id", str(next_auto_rail_id))
                    ...
                    created_auto_rails[position] = next_auto_rail_id
                    next_auto_rail_id += 1
            ...
            for i, fma in enumerate(fmas):
                ...
                if fma.get("associated_rails"):
                    for rail_ref in fma["associated_rails"]:
                        ...
                        if not rid:
                            continue
                        # Ignorar rails automáticos em memória (ex.: 7002) e quaisquer ids que não estejam no XML
                        if rid in real_rail_ids_in_xml:
                            if ref_id not in added_ref_ids:
                                ET.SubElement(rails_refs_xml, "Ref", {"refId": rid})
                                added_ref_ids.add(rid)
                
                # Adicionar rail automático se FMA está numa posição sem rail real
                fma_position = (fma.get("x"), fma.get("y"))
                real_here = self._find_real_rail_at_position(real_rails + switches, fma_position)

                # FMAs sempre devem ter um rail, mesmo que não exista sensor na mesma posição
                if not real_here:
                    # Se não há rail automático criado pelos sensores, criar um para a FMA
                    if fma_position not in created_auto_rails:
                        # Criar rail automático específico para FMA
                        ...
                        created_auto_rails[fma_position] = next_auto_rail_id
                        next_auto_rail_id += 1
                    
                    # referenciar o rail automático criado/reutilizado (600x)
                    ...
                    # Se há rail real, associar à FMA
                    rid = str(real_here.get("id"))
                    if rid and rid not in added_ref_ids:
                        ET.SubElement(rails_refs_xml, "Ref", {"refId": rid})
                        added_ref_ids.add(rid)

                # Também verificar se havia rail automático criado por sensor na mesma posição
            ...
            try:
                ...
                if track_elem is not None:
                    rails_and_links = [el for el in track_elem if el.tag in ("Rail", "Link")]
...
class FDSFormGenerator:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("Praxis - Sistema de Configuração Ferroviária")
    ...
    def create_fds_config_tab(self):
        ...
        self.create_form_field_styled(row1, "ConfigVersion", "1.0", "Versão da Configuração:", True, True)
        ...
        self.create_form_field_styled(net_row1, "IpAddressNet1", "192.168.1.27", "IP Rede 1:", True, False)
        ...
        self.create_form_field_styled(net_row3, "GatewayAddressNet1", "192.168.1.27", "Gateway Rede 1:", True, False)
        ...
        ttk.Button(btn_row1, text="➕ TrackSection", command=self.add_track_section, 
                  style='Corporate.TButton').pack(side=tk.LEFT, padx=(0, 10))
        ttk.Button(btn_row1, text="➖ Remover Elemento", command=self.remove_element, 
                  style='Corporate.TButton').pack(side=tk.LEFT, padx=(0, 10))
        ...
        ttk.Label(search_frame, text="Pesquisar por Element ID:", 
    ...
    def load_fds_config(self):
        ...
        try:
            ...
            self.form_fields["TimeZone"].set(root.findtext("TimeZone", default="CET-1CEST-2,M3.5.0/02:00:00,M10.5.0/03:00:00"))
    ...
    def load_elements_from_xml(self, root):
        ...
        """
        try:
            print("=== INICIANDO CARREGAMENTO DE ELEMENTOS (compatível c/ versões) ===")
    ...
    def show_grid_config(self):
        ...
        try:
            ...
            ttk.Button(btn_frame, text="Aplicar", command=dialog.destroy).pack(side=tk.LEFT, padx=(0, 10))
    ...
    def apply_grid(self):
        ...
        # Desenhar linhas da grade APENAS se visível
        ...
        # Números das colunas (sempre visíveis) - margem superior
        ...
        # Números das linhas (sempre visíveis) - margem esquerda
        ...
        # Canto superior esquerdo (sempre visível)
    ...
    def add_automatic_rail_for_element(self, grid_x, grid_y, element_type, element_angle, element_id=None):
        ...
        try:
            ...
            if existing_auto_rail:
                ...
                element_id_rail = 6000 + (self.next_element_id % 1000)  # Garantir range 6000-6999
                rail_element = {
                    "type": "rail",
                    "id": element_id_rail,
                    "x": grid_x,
    ...
    def save_state_for_undo(self):
        ...
        try:
            ...
            # Criar estado limpo sem referências tkinter
            clean_elements = []
            for element in getattr(self, 'trackplan_elements', []):
                # Criar cópia limpa
                clean_element = {}
                safe_fields = ['type', 'id', 'x', 'y', 'angle', 'mirror', 'name', 'ref_id',
                            'url', 'fma0', 'fma1', 'fma_type', 'position', 'rail_type',
                            'associated_sensors', 'associated_rails', 'is_auto_rail']
                
                for field in safe_fields:
                    if field in element:
                        if field in ['associated_sensors', 'associated_rails']:
                            clean_element[field] = [item.copy() if isinstance(item, dict) else item 
                                                for item in element[field]]
                        else:
                            clean_element[field] = element[field]
                
                clean_elements.append(clean_element)
            
            state = {
                'elements': clean_elements,
                'width': getattr(self, 'width_var', tk.StringVar()).get(),
                'height': getattr(self, 'height_var', tk.StringVar()).get()
            }
        ...
        try:
            ...
            if clicked_element:
                ...
                if is_selected:
                    ...
                    print(f"Elemento desselecionado")
                else:
    ...
    def apply_grid(self):
        ...
        # Calcular dimensões reais: max_col=71 significa colunas 0-71, então precisamos de 72 colunas
        actual_width = max_col + 1
        actual_height = max_row + 1
        ...
        # Números das colunas (clicáveis) - margem superior
        for x in range(actual_width):
            x_pos = (x + 1) * self.grid_size + self.grid_size // 2
            rect_id = self.trackplan_canvas.create_rectangle(
                (x + 1) * self.grid_size, 0, (x + 2) * self.grid_size, self.grid_size,
        ...
        # Números das linhas (clicáveis) - margem esquerda
        for y in range(actual_height):
            y_pos = (y + 1) * self.grid_size + self.grid_size // 2
            rect_id = self.trackplan_canvas.create_rectangle(
                0, (y + 1) * self.grid_size, self.grid_size, (y + 2) * self.grid_size,
    ...
    def copy_selection(self):
        ...
        try:
            ...
            # Filtrar apenas elementos reais (não células vazias)
            filtered_elements = [elem for elem in self.selected_elements if isinstance(elem, dict)]
            
            if not filtered_elements:
                messagebox.showinfo("Aviso", "Nenhum elemento válido selecionado para copiar.")
                return
            
            # Criar cópias limpas sem referências tkinter
            copied_elements = []
            for element in filtered_elements:
                # Criar cópia limpa do elemento
                clean_element = {}
                
                # Campos seguros para copiar
                safe_fields = ['type', 'id', 'x', 'y', 'angle', 'mirror', 'name', 'ref_id', 
                            'url', 'fma0', 'fma1', 'fma_type', 'position', 'rail_type',
                            'associated_sensors', 'associated_rails']
                
                for field in safe_fields:
                    if field in element:
                        if field in ['associated_sensors', 'associated_rails']:
                            # Para listas, fazer cópia manual
                            clean_element[field] = [item.copy() if isinstance(item, dict) else item 
                                                for item in element[field]]
                        else:
                            clean_element[field] = element[field]
                
                copied_elements.append(clean_element)
            
            # Encontrar ponto de origem (menor x, menor y)
            if copied_elements:
                min_x = min(elem.get('x', 0) for elem in copied_elements)
                min_y = min(elem.get('y', 0) for elem in copied_elements)
                ...
                messagebox.showinfo("Sucesso", f"{len(copied_elements)} elemento(s) copiado(s)!")
    ...
    def cut_selection(self):
        ...
        try:
            ...
            # Salvar estado para undo ANTES de fazer alterações
            ...
            # Deletar apenas os elementos reais
    ...
    def paste_selection(self):
        ...
        try:
            ...
            # Verificar estrutura do clipboard
        ...
        try:
            ...
            if element_type == 'rail':
                ...
                new_element = self.add_fma_element(
                    grid_x, grid_y, 
                    element_data.get('angle', 0), 
                    element_data.get('name', f'FMA{self.next_element_id}'),
                    fma_type
                )
    ...
    def create_trackplan_designer_tab(self):
        ...
        for text, value in [("Trilho", "rail"), ("Link", "link"), ("Chave", "switch"), ("Sensor", "sensor"), 
    ...
    def create_cubicles_tab(self):
        ...
        ttk.Button(buttons_frame, text="Remover", command=self.remove_cubicle).pack(side=tk.LEFT, padx=4)
        ...
        columns = ("ID", "Nome", "Slots", "AEBs", "Status")
        ...
        self.cubicles_tree.heading("Status", text="Status")
        ...
        self.cubicles_tree.column("Status", width=80, anchor="center")
    ...
    def create_cubicle_exportation_tab(self):
        ...
        # Frame de validação
        validation_checks_frame = ttk.LabelFrame(scrollable, text="Validações", padding="10")
        validation_checks_frame.pack(fill=tk.X, padx=10, pady=5)
        
        # Texto de validação
        self.validation_text = tk.Text(validation_checks_frame, height=15, width=80)
        validation_scrollbar = ttk.Scrollbar(validation_checks_frame, orient="vertical", command=self.validation_text.yview)
        ...
        ttk.Button(actions_frame, text="Gerar XML", command=self.generate_cubicle_xml).pack(side=tk.LEFT, padx=5)
        ...
        self.xml_preview_text = tk.Text(preview_frame, height=10, wrap=tk.NONE)
    ...
    def remove_cubicle(self):
        ...
        if messagebox.askyesno("Confirmar", "Tem certeza que deseja remover este cubicle?"):
            item_id = selection[0]
    ...
    def draw_rack_layout(self):
        ...
        if not self.rack_slots:
            self.rack_canvas.create_text(300, 200, text="Nenhum cubicle selecionado", font=("Arial", 12), fill="gray")
            return
        ...
        if slot_type == 'Com':
            ...
            try:
                ...
                if slot_id_text.startswith('5') and len(slot_id_text) >= 2:
                    ...
                    ref_id = f'1{base_number}'  # Adiciona prefixo "1" (mesmo do sensor)
                else:
    ...
    def validate_cubicle(self):
        """Valida o cubicle atual"""
        if not self.current_cubicle:
            self.validation_text.delete(1.0, tk.END)
            self.validation_text.insert(tk.END, "Nenhum cubicle selecionado para validação.\n")
            return
        
        self.validation_text.delete(1.0, tk.END)
        self.validation_text.insert(tk.END, f"VALIDANDO CUBICLE: {self.current_cubicle.get('name', 'N/A')}\n")
        ...
        self.validation_text.insert(tk.END, "  DADOS BÁSICOS:\n")
        if not self.current_cubicle.get('id'):
            self.validation_text.insert(tk.END, "❌ ID do cubicle não definido\n")
            errors += 1
        else:
            self.validation_text.insert(tk.END, f"✅ ID: {self.current_cubicle['id']}\n")
        
        if not self.current_cubicle.get('name'):
            self.validation_text.insert(tk.END, "❌ Nome do cubicle não definido\n")
            errors += 1
        else:
            self.validation_text.insert(tk.END, f"✅ Nome: {self.current_cubicle['name']}\n")
        
        # Validar rack
        self.validation_text.insert(tk.END, "\n   ESTRUTURA DO RACK:\n")
        rack = self.current_cubicle.get('rack', {})
        if not rack.get('id'):
            self.validation_text.insert(tk.END, "❌ ID do rack não definido\n")
            errors += 1
        else:
            self.validation_text.insert(tk.END, f"✅ Rack ID: {rack['id']}\n")
        
        # Validar backplane
        bp = rack.get('bp', {})
        if not bp.get('id'):
            self.validation_text.insert(tk.END, "❌ ID do backplane não definido\n")
            errors += 1
        else:
            self.validation_text.insert(tk.END, f"✅ Backplane ID: {bp['id']}\n")
        
        # Validar slots
        self.validation_text.insert(tk.END, "\n   SLOTS:\n")
        slots = bp.get('slots', {})
        
        # Verificar PSC no slot 1
        if '1' not in slots or slots['1'].get('type') != 'Psc':
            self.validation_text.insert(tk.END, "❌ PSC deve estar no Slot 1\n")
            errors += 1
        else:
            self.validation_text.insert(tk.END, "✅ PSC no Slot 1\n")
        
        # Contar componentes
        component_counts = {}
        for slot in slots.values():
            slot_type = slot.get('type', 'EmptySlot')
            component_counts[slot_type] = component_counts.get(slot_type, 0) + 1
        
        for comp_type, count in component_counts.items():
            if comp_type != 'EmptySlot':
                self.validation_text.insert(tk.END, f"{comp_type}: {count} unidades\n")
        ...
        for slot in slots.values():
            slot_id = slot.get('id', '')
            if slot_id:
                if slot_id in ids_used:
                    duplicate_ids.append(slot_id)
                else:
                    ids_used.append(slot_id)
        
        if duplicate_ids:
            self.validation_text.insert(tk.END, f"❌ IDs duplicados encontrados: {', '.join(duplicate_ids)}\n")
            errors += len(duplicate_ids)
        else:
            self.validation_text.insert(tk.END, "✅ Todos os IDs são únicos\n")
        
        # Verificar referências AEB-FdsConfig
        self.validation_text.insert(tk.END, "\n   REFERÊNCIAS AEB-FDSCONFIG:\n")
        aeb_refs = []
        for slot in slots.values():
            if slot.get('type') == 'Aeb' and slot.get('refId'):
                aeb_refs.append(slot['refId'])
        
        if aeb_refs:
            self.validation_text.insert(tk.END, f" AEBs encontradas: {', '.join(aeb_refs)}\n")
        
        # Resultado final
        self.validation_text.insert(tk.END, "\n" + "=" * 50 + "\n")
        self.validation_text.insert(tk.END, f"RESULTADO DA VALIDAÇÃO:\n")
        self.validation_text.insert(tk.END, f"Erros: {errors}\n")
        self.validation_text.insert(tk.END, f"Avisos: {warnings}\n")
        
        if errors == 0:
            self.validation_text.insert(tk.END, "\n✅ CUBICLE VÁLIDO PARA EXPORTAÇÃO!\n")
        else:
            self.validation_text.insert(tk.END, "\n❌ CUBICLE POSSUI ERROS - CORRIJA ANTES DE EXPORTAR\n")
    ...
    def generate_all_cubicles_xml(self):
        ...
        try:
            ...
            # Ordenar cubicles por ID para manter consistência
            sorted_cubicles = sorted(self.cubicles_data, key=lambda x: x.get('id', ''))
            ...
            for slot_id, slot in sorted_slots:
                    ...
                    if slot_type == 'Psc':
                        ...
                        attributes = [
                            ...
                            f'type="COM_FSE"',  # Sempre COM_FSE
                            f'redundant="NORMAL"',  # Sempre NORMAL
            ...
            messagebox.showinfo("Sucesso", f"XML gerado para {len(sorted_cubicles)} cubículos!")
            
        except Exception as e:
            self.xml_preview_text.delete(1.0, tk.END)
            self.xml_preview_text.insert(tk.END, f"Erro ao gerar XML: {str(e)}")
    
    def generate_cubicle_xml(self):
        ...
        try:
            ...
            xml_lines = []
            xml_lines.append(f'\t\t<Cubicle height="{self.current_cubicle.get("height", "1")}" id="{self.current_cubicle.get("id", "")}" name="{self.current_cubicle.get("name", "")}"> ')
            ...
            for slot_id, slot in sorted_slots:
                slot_type = slot.get('type', 'EmptySlot')
                slot_xml_id = slot.get('id', '')
                xml_lines.append(f'\t\t\t\t\t<Slot id="{slot_xml_id}" slotId="{slot_id}">')
                
                if slot_type == 'Psc':
                    xml_lines.append(f'\t\t\t\t\t\t<Psc/>')
                elif slot_type == 'Com':
                    name = slot.get('name', '')
                    can_id = slot.get('canId', '')
                    type_com = slot.get('type_com', 'COM_FSE')
                    redundant = slot.get('redundant', 'NORMAL')
                    xml_lines.append(f'\t\t\t\t\t\t<Com name="{name}" canId="{can_id}" type="{type_com}" redundant="{redundant}"/>')
                elif slot_type == 'Aeb':
                    name = slot.get('name', '')
                    can_id = slot.get('canId', '')
                    ref_id = slot.get('refId', '')
                    xml_lines.append(f'\t\t\t\t\t\t<Aeb name="{name}" canId="{can_id}" refId="{ref_id}"/>')
                elif slot_type == 'IoExb':
                    name = slot.get('name', '')
                    ref_id = slot.get('refId', '')
                    xml_lines.append(f'\t\t\t\t\t\t<IoExb name="{name}" refId="{ref_id}"/>')
                else:
                    xml_lines.append(f'\t\t\t\t\t\t<EmptySlot/>')
                
                xml_lines.append(f'\t\t\t\t\t</Slot>')
    ...
    def create_fma_config_window(self):
        ...
        ttk.Button(rails_buttons, text="➕ Adicionar Trilho", command=self.add_rail_to_fma).pack(side=tk.LEFT, padx=5)
    ...
    def add_sensor_to_fma(self):
        """Adiciona sensor à FMA por seleção no canvas"""
        messagebox.showinfo("Info", "Clique no canvas em um sensor para adicioná-lo à FMA.\nEscolha a posição (left/right) na próxima etapa.")
        self.fma_config_window.withdraw()  # Minimizar janela temporariamente
        self.trackplan_canvas.bind("<Button-1>", self.select_sensor_for_fma)
    
    def select_sensor_for_fma(self, event):
        try:
            ...
            grid_x = int((canvas_x - self.grid_size) // self.grid_size)
            grid_y = int((canvas_y - self.grid_size) // self.grid_size)
            ...
            position = self.show_sensor_position_dialog(sensor_element)
    ...
    def show_sensor_position_dialog(self, sensor_element):
        ...
        # Inicializar preview com posição padrão (direita)
        self.update_sensor_preview(preview_canvas, sensor_element, 'right')
        ...
        ttk.Button(button_frame, text="✓ Confirmar", command=on_confirm, width=15).pack(side=tk.RIGHT, padx=(10, 0))
    ...
    def add_rail_to_fma(self):
        """Adiciona trilho à FMA por seleção no canvas"""
        self.fma_config_window.withdraw()  # Minimizar janela temporariamente
        self.trackplan_canvas.bind("<Button-1>", self.select_rail_for_fma)

    def select_rail_for_fma(self, event):
        """Seleciona trilho clicado para adicionar à FMA"""
        try:
            # Converter coordenadas
            canvas_x = self.trackplan_canvas.canvasx(event.x)
            canvas_y = self.trackplan_canvas.canvasy(event.y)
            ...
            # Procurar trilho na posição
            rail_element = None
            for element in self.trackplan_elements:
                if element.get('type') in ['rail', 'switch', 'link'] and element.get('x') == grid_x and element.get('y') == grid_y:
                    rail_element = element
                    break
            
            self.fma_config_window.deiconify()

            if not rail_element:
                messagebox.showwarning("Aviso", "Nenhum trilho encontrado na posição clicada.")
                return

            # Verificar se este trilho já está associado à FMA atual
            rail_id = str(rail_element.get('id', ''))
            existing_rails = self.current_fma_element.get('associated_rails', [])
            if any(r.get('refId') == rail_id for r in existing_rails):
                messagebox.showwarning("Trilho já adicionado", f"O trilho {rail_id} já está associado a esta FMA.")
                return

            rail_data = {'refId': rail_id}
            if 'associated_rails' not in self.current_fma_element:
                self.current_fma_element['associated_rails'] = []
            self.current_fma_element['associated_rails'].append(rail_data)
            self.rails_listbox.insert(tk.END, rail_data['refId'])
            messagebox.showinfo("Sucesso", f"Trilho {rail_data['refId']} adicionado à FMA.")
        finally:
            # Sempre restaurar bindings e foco do canvas
            self.trackplan_canvas.unbind("<Button-1>")
            self.restore_canvas_bindings()
    ...
    def highlight_selected_rail(self):
        ...
        for element in self.trackplan_elements:
            if element.get('type') in ['rail', 'switch', 'link'] and str(element.get('id', '')) == rail_ref_id:
    ...
    def highlight_element(self, element, temporary=False, duration_ms=3000, fma_position=None, color=None):
        ...
        try:
            if element_type in ['rail', 'switch', 'link']:
    ...
    def create_sensor_config_window(self):
        ...
        # Possui FMA?
        row += 1
        initial_has_fma = bool(self.current_sensor_element.get('fma0')) and bool(self.current_sensor_element.get('fma1'))
    ...
    def update_sensor_auto_fields(self, *args):
        ...
        if sensor_id_input:
            ...
            if hasattr(self, 'current_sensor_element') and self.current_sensor_element:
                ...
                self.update_sensor_name_on_canvas()
        else:
            # Limpar campos se ID estiver vazio
            self.sensor_name_var.set('')
            self.sensor_ref_id_var.set('')
    ...
    def delete_selection(self):
        ...
        # Filtrar apenas elementos reais (não posições vazias)
        real_elements = [elem for elem in self.selected_elements if elem.get('type') != 'empty_position']
        
        if not real_elements:
            messagebox.showwarning("Aviso", "Nenhum elemento selecionado para remover.")
            return
        
        # Salvar estado para undo
        self.save_state_for_undo()
        
        # Remover elementos selecionados (apenas os reais)
        self.clear_selection()
        
        # Remover elementos do canvas
        for element in real_elements:
            self.delete_element_completely(element)
            if element in self.trackplan_elements:
                self.trackplan_elements.remove(element)
        
        messagebox.showinfo("Sucesso", f"{len(real_elements)} elemento(s) removido(s).")
    ...
    def select_all(self):
        ...
        if self.trackplan_elements:
            messagebox.showinfo("Sucesso", f"Todos os {len(self.trackplan_elements)} elementos foram selecionados.")
        else:
    ...
    def _update_next_element_id_from_loaded_elements(self):
        """Atualiza next_element_id baseado no maior ID encontrado nos elementos carregados"""
        try:
            max_id = 0
            elements_analyzed = 0
                        
            # Verificar todos os elementos carregados
            for element in self.trackplan_elements:
                ...
                if element_id is not None:
                    try:
                        ...
                        if isinstance(element_id, str):
                            # Para IDs como "31230", "41230", etc., usar o valor completo
                        ...
                        print(f"ID não numérico ignorado: {element_id} (elemento: {element.get('type', 'unknown')})")
                        continue
            
            # Verificar também os elementos FDS se existirem
            if hasattr(self, 'current_generator') and self.current_generator:
                for fds_element in self.current_generator.elements:
                    try:
                        fds_id = int(fds_element.element_id)
                        if fds_id > max_id:
                            max_id = fds_id
                        elements_analyzed += 1
            ...
            # Definir next_element_id
            if max_id > 0:
                # Usar o próximo ID disponível após o maior encontrado
                self.next_element_id = max_id + 1
            else:
                # Se nenhum ID válido foi encontrado, usar valor padrão
                self.next_element_id = 7001
                        
        except Exception as e:
            print(f"Erro ao atualizar next_element_id: {e}")
            # Em caso de erro, usar valor seguro
            self.next_element_id = 9999
        ...
        for sensor_data in xml_data.sensors:
            try:
                ...
                if existing_rail:
                    ...
                    print(f"Sensor {sensor_data.name} na posição {position} (sem rail)")
    ...
    def initialize_generator(self):
        ...
        try:
            ...
            network = NetworkConfig(
                ip_net1=self.form_fields.get("IpAddressNet1", tk.StringVar()).get() or "192.168.1.27",
                ...
                gateway_net1=self.form_fields.get("GatewayAddressNet1", tk.StringVar()).get() or "192.168.1.27",
    ...
    def update_elements_tree(self):
        ...
        if self.current_generator:
            ...
            for element in sorted_elements:
                # Filtrar por ID se houver texto de pesquisa
                if not search_text or str(element.element_id).lower().startswith(search_text):
                    # Formatação visual: IDs com prefixo por tipo + base 3 dígitos
                    try:
                        base_id = int(element.element_id)
                    except Exception:
                    ...
                    # Aplicar prefixos para todos os elementos
                    type_to_prefix = {
                        'ComMaster': '0',
                        'Aeb': '1',
                        'CountingHead': '2',
                    ...
                    display_id_with_prefix = f"{prefix}{base_id:03d}" if isinstance(base_id, int) and prefix != '' else str(base_id)

                    # Element ID SEM prefixo (coluna "ID")
                    element_id_raw = str(element.element_id)

                    element_assignment = getattr(element, 'element_assignment', '')

                    self.elements_tree.insert(
                        "", "end",
                        values=(
                            element_id_raw,           # Coluna "ID" - sempre sem prefixo
                            element.element_type,     # Coluna "Tipo"
                            element.element_assignment, # Coluna "Atribuição"
                            display_id_with_prefix    # Coluna "TrackplanID" - com prefixo conforme tipo
                        )
                    )
    ...
    def on_canvas_click(self, event):
        ...
        if tool == "rail":
            ...
            url = simpledialog.askstring("URL do Link", "Digite o IP do outro Trackplan:")
            if url:
    ...
    def add_rail_element(self, grid_x, grid_y, angle, mirror, preserve_id=None):
        """Adiciona um elemento de trilho"""
        # Se preserve_id for fornecido, usar ele; senão, gerar novo ID
    ...
    def add_switch_element(self, grid_x, grid_y, angle, mirror, preserve_id=None):
    ...
    def add_link_element(self, grid_x, grid_y, angle, url, preserve_id=None):
    ...
    def update_sensor_fma_associations(self):
        ...
        try:
            ...
            for sensor in sensors:
                ...
                if bucket and bucket.get("3") and bucket.get("4"):
                    sensor["fma0"] = bucket["3"]
                    sensor["fma1"] = bucket["4"]
                else:
                    # Preservar o que já está no sensor (não apagar)
                    if existing_fma0:
    ...
    def add_fma_element(self, grid_x, grid_y, angle, name=None, fma_type='FMA1', ref_id=None):
        """Adiciona um elemento FMA com linha e quadrado de texto conforme ângulo especificado"""
        # Gerar ID baseado no tipo
        base_id = self.next_element_id
        if fma_type == 'FMA2':
            element_id = f"4{base_id}"
        else:  # FMA1 (padrão)
            element_id = f"3{base_id}"

        self.next_element_id += 1
        
        # Usar nome fornecido ou gerar padrão
        if name:
            fma_name = name
        else:
            # Extrair número base do ID para o nome
            fma_name = name if name else f"FMA{str(element_id)[1:]}"
        
        # Definir ref_id da FMA (deve apontar para um sensor)
        if ref_id:
            # Usar ref_id fornecido
            fma_ref_id = str(ref_id)
        else:
            # Tentar encontrar sensor na mesma posição ou próximo
            sensor_nearby = self._find_sensor_at_or_near_position(grid_x, grid_y)
            if sensor_nearby:
                fma_ref_id = str(sensor_nearby.get("id", ""))
            else:
                base = str(element_id)[1:]
                fma_ref_id = f"2{base}"
        ...
        # Gerar imagem com texto integrado (sempre)
        if image_key not in self.element_images:
            custom_image = self.create_fma_image_with_integrated_text(angle, fma_name)
            if custom_image:
                self.element_images[image_key] = custom_image
            else:
                print(f"FMA: Erro ao gerar imagem '{image_key}'")
        else:
            print(f"FMA: Reutilizando imagem existente '{image_key}'")
        
        # Usar imagem com texto
        if image_key in self.element_images:
            fma_canvas_id = self.trackplan_canvas.create_image(
                canvas_x, canvas_y, 
                image=self.element_images[image_key], 
                tags="fma"
            )
            # Adicionar aos elementos (usando imagem)
            fma_element = {
                "type": "fma",
                "id": element_id,
                "x": grid_x,
                "y": grid_y,
                "canvas_id": fma_canvas_id,
                "box_id": None,  # Imagem já inclui caixa
                "text_id": None,  # Imagem já inclui texto
                "name": fma_name,
                "ref_id": fma_ref_id,  # ref_id correto apontando para sensor
                "angle": angle,
                "fma_type": fma_type, # Adicionar tipo de FMA (FMA1 ou FMA2)
                "image_key": image_key,  # Para redo/undo
                "associated_rails": [] # Inicia a lista vazia
            }
        else:
            # Fallback: usar desenho por código se imagem não funcionar
            # 1. Desenhar linha do FMA (4px de largura correndo os 30px completos)
            line_coords = self.get_fma_line_coords_full(canvas_x, canvas_y, angle)
            line_id = self.trackplan_canvas.create_line(*line_coords, fill="black", width=4, tags="fma")
            
            # 2. Desenhar caixa para o texto (posicionada de acordo com ângulo)
            box_coords = self.get_fma_box_coords_by_angle(canvas_x, canvas_y, angle, fma_name)
            box_id = self.trackplan_canvas.create_rectangle(*box_coords, 
                fill="white", outline="black", width=1, tags="fma")
            
            # 3. Desenhar texto PRETO centralizado na caixa
            text_x, text_y = self.get_fma_text_position_in_box(box_coords)
            text_id = self.trackplan_canvas.create_text(
                text_x, text_y,
                text=fma_name,
                font=("Arial", 5, "bold"),  # Fonte pequena para caber na caixa
                fill="black",               # Texto preto
                anchor="center",            # Centralizado
                tags="fma"
            )
            
            # Adicionar aos elementos (usando desenho por código)
            fma_element = {
                "type": "fma",
                "id": element_id,
                "x": grid_x,
                "y": grid_y,
                "canvas_id": line_id,  # Linha
                "box_id": box_id,
                "text_id": text_id,
                "name": fma_name,
                "ref_id": fma_ref_id,  # ref_id para associação com sensores
                "angle": angle,
                "fma_type": fma_type,
                "associated_rails": [] 
            }
        ...
        # Rail automático para a FMA (apenas em memória; não desenha)
        auto_rail = None
        for e in self.trackplan_elements:
        ...
        if auto_rail:
            ...
            # Adicionar rail à lista de trilhos associados da FMA
            fma_element["associated_rails"].append({
                "refId": rail_id,
                "auto": True  # Marcar como rail automático
            })

        # Incrementar APENAS AGORA, após criar rail automático
        self.next_element_id += 1

        # Atualizar associações sensor-FMA
        try:
            self.update_sensor_fma_associations()
        except Exception:
            pass
    ...
    def remove_element_at_position(self, grid_x, grid_y, preserve_auto_rails=False):
        ...
        """
        try:
            # Salvar estado para undo ANTES de fazer qualquer alteração
            self.save_state_for_undo()
    ...
    def undo_action(self):
        ...
        try:
            ...
            current_state = {
                'elements': copy.deepcopy(self.trackplan_elements),
    ...
    def delete_element_completely(self, element):
        ...
        try:
            ...
            if element_type in ['link', 'rail', 'switch', 'sensor']:
                self._dissociate_element_from_all_fmas(element)
                ...
                if hasattr(self, 'fma_blue_cache'):
                    ...
                    for key in blue_keys_to_remove:
                        try:
                            ...
                            print(f"🧹 Cache FMA azul removido: {key}")
                        except Exception:
                            pass
    ...
    def _dissociate_element_from_all_fmas(self, element):
        ...
        try:
            ...
            if not eid or etype not in ('rail', 'switch', 'link', 'sensor'):
                return
            ...
            for fma in getattr(self, 'trackplan_elements', []):
                ...
                # Remover de associated_rails se for rail/switch/link
                if etype in ('rail', 'switch', 'link'):
    ...
    def save_config(self):
        ...
        if filename:
            try:
                ...
                fds_xml = self.current_generator.generate_fds_config_xml()
        ...
        try:            
            ...
            try:                
                ...
                if hasattr(self, 'current_generator') and self.current_generator:
                    try:
                        self.update_generator_from_form()
                        fds_xml_content = self.current_generator.generate_fds_config_xml()
                    except Exception as e:
    ...
    def generate_current_fds_config_xml(self):
        ...
        try:
            ...
            # Verificar se há FdsConfig carregado
            if hasattr(self, 'fds_config_data') and self.fds_config_data and 'root' in self.fds_config_data:                
                # Usar o FdsConfig carregado como base
                loaded_root = self.fds_config_data['root']
                
                # Criar nova estrutura baseada no carregado
                root = ET.Element("FdsConfig")
                
                # Copiar todos os elementos do FdsConfig carregado
                for child in loaded_root:
                    if child.tag != "ElementList":
                        # Copiar configurações básicas
                        new_elem = ET.SubElement(root, child.tag)
                        new_elem.text = child.text
                        # Copiar atributos se existirem
                        for attr_name, attr_value in child.attrib.items():
                            new_elem.set(attr_name, attr_value)
                
                # Atualizar com dados dos campos do formulário se preenchidos
                self._update_basic_config_from_form(root)
                
                # Adicionar ElementList do FdsConfig carregado
                elements_elem = ET.SubElement(root, "ElementList")
                loaded_elements = loaded_root.find("ElementList")
                
                if loaded_elements is not None:
                    elements_added = 0
                    for element in loaded_elements.findall("Element"):
                        # Copiar elemento completo
                        new_element = ET.SubElement(elements_elem, "Element")
                        
                        for elem_child in element:
                            new_child = ET.SubElement(new_element, elem_child.tag)
                            new_child.text = elem_child.text
                            # Copiar atributos se existirem
                            for attr_name, attr_value in elem_child.attrib.items():
                                new_child.set(attr_name, attr_value)
                        
                        elements_added += 1
                    
                else:
                    print("ElementList não encontrado no FdsConfig carregado")
                
            else:
                print("Nenhum FdsConfig carregado, gerando com dados do formulário")
                
                # Criar estrutura XML básica com dados do formulário
                root = ET.Element("FdsConfig")
                
                # Informações básicas dos campos do formulário
                self._add_basic_config_from_form(root)
                
                # Lista de elementos do elements_tree ou padrão
                elements_elem = ET.SubElement(root, "ElementList")
                elements_added = self._add_elements_from_tree(elements_elem)
            
            # Converter para string formatada sem inserir espaços extras
            try:
                tree = ET.ElementTree(root)
                # Python 3.9+: indenta sem criar nós de texto em branco
    ...
    def _update_basic_config_from_form(self, root):
        """Atualiza configurações básicas do XML com dados do formulário"""
        form_mapping = {
            "ConfigVersion": "ConfigVersion",
            "IpAddressNet1": "IpAddressNet1", 
            "MaskNet1": "MaskNet1",
            "IpAddressNet2": "IpAddressNet2",
            "MaskNet2": "MaskNet2", 
            "GatewayAddressNet1": "GatewayAddressNet1",
            "GatewayAddressNet2": "GatewayAddressNet2",
            "UdpPortFadc": "UdpPortFadc",
        ...
        for xml_tag, form_field in form_mapping.items():
            if form_field in self.form_fields:
                form_value = self.form_fields[form_field].get()
                if form_value:  # Só atualizar se há valor no formulário
                    existing_elem = root.find(xml_tag)
                    if existing_elem is not None:
                        existing_elem.text = form_value
                    else:
                        ET.SubElement(root, xml_tag).text = form_value
    ...
    def generate_trackplan_xml(self):
        ...
        try:
            ...
            xml_generator.generate_smart_xml(temp_filename)
            ...
            try:
                ...
                if hasattr(self, 'fds_xml_text'):
                    ...
                    if hasattr(self, 'current_generator') and self.current_generator:
                        try:
                            self.update_generator_from_form()
                            xml_content = self.current_generator.generate_fds_config_xml()
                        except Exception as e:
    ...
    def get_xsd_file_path(self, schema_type):
        """Retorna caminho para o arquivo XSD"""
        # Diretório base do script
        base_dir = os.path.dirname(os.path.abspath(__file__))
        
        # Caminhos possíveis para os XSDs
        possible_paths = [
            # Pasta Referência/XSD (onde você colocou)
            os.path.join(base_dir, "..", "Referência", "XSD", f"{schema_type.capitalize()}.xsd"),
            # Pasta atual
            os.path.join(base_dir, f"{schema_type.capitalize()}.xsd"),
            # Pasta XSD na raiz do projeto
    ...
    def validate_xml(self):
        ...
        try:
            ...
            if missing_xsds:
                ...
                error_msg += "\n\nPor favor, certifique-se de que os arquivos XSD estão na pasta Referência/XSD/"
            ...
            try:
                ...
                self.validation_results.insert(tk.END, f"❌ Erro ao validar FdsConfig.xml: {e}\n")
                errors_found += 1
    ...
    def show_xsd_locations(self):
        ...
        info += "Coloque os arquivos XSD na pasta 'Referência/XSD/' para melhor organização."        
        result = messagebox.askyesno("Localização dos XSDs", 
                            info + "\n\nDeseja abrir a pasta dos XSDs no Explorer?")
    
        if result:
            # Abrir pasta Referência/XSD no Explorer
            xsd_folder = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "Referência", "XSD")
    ...
    def _get_trackplan_info(self, content):
        ...
        try:
            ...
            info_lines.append(f"   • Trilhos/Switches: {len(rails)}")
